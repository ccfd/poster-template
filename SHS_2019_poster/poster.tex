...
In this contribution, we present a conservative, phase-field model for simulation of immiscible multiphase flows using an incompressible, velocity based Cascaded Lattice Boltzmann Method (CLBM). 
As a study case, we present a simulation of a SHS.
...
%The lattice Boltzmann method, thanks to its mesoscopic nature is a suitable candidate to simulate the intermolecular interactions responsible for the interfacial flows in a microscale. 
%Additionally, the LBM is easy in implementation and in ability of handling complex geometries such as porous media or surface roughness. 
%Computations are performed locally, thus the algorithm can be parallelized in a straightforward manner. 
%These features resulted in raising popularity and a commercial success of the method. 
%Application of LB method to the simulation of SHS is a recent research field. 
\end{block}
...
Such symmetric structures are expensive to manufacture and not common in nature. 
However, experiments show that an unstructured rough surface can work as well as regularly spaced arrays of ridges or posts.
...
\begin{block}{Algorithm: hydrodynamics}
\textit{1} Initialize $ \boldsymbol{f}(\textbf{x}, t)  $ \\ \vspace{2.5em}

\textit{2} Compute 
$\textbf{u} = [u_x, u_y]^\top = [ k_{10}, k_{01}]^\top 
= \sum_\alpha f_\alpha \textbf{e}_\alpha + \frac{\textbf{F}}{2 \rho} \delta t \nonumber  $ \\ \vspace{2.5em}
...
Interface location: $ \phi_0 =(\phi_H + \phi_L)/2  $ \\
...
\textit{1} Initialize 
$ \boldsymbol{h}(\textbf{x}, t) $ \\ \vspace{2.5em}
%\pause

\textit{2} Use $ \boldsymbol{u}$ from hydrodynamics  \\ \vspace{2.5em}
...
\begin{figure}[H]
   \includegraphics[width= 0.8 \textwidth]{Images/kanal_z_u_profile.jpg} 
...
\begin{block}{Summary}
In this contribution, we present a conservative, phase-field model for simulation of immiscible multiphase flows using an incompressible, velocity based Cascaded Lattice Boltzmann Method (CLBM). We introduce improvements to the LB equations for interface tracking and incompressible hydrodynamics, by transforming the operations into the central moment space. The relaxation of central moments is defined in a reference frame moving with the fluid, while the previously used multiple-relaxation time scheme performs collision in a fixed reference frame. As a consequence, CLBM enhances Galilean invariance and stability of the method. As a study case, we present a simulation of a SHS.
